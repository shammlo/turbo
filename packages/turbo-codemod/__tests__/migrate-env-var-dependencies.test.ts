  ...
  describe("transform", () => {
    it("migrates turbo.json env var dependencies - basic", async () => {
      ...
      const { root, read, cleanup } = testTransform({
        fixture: "migrate-env-var-dependencies/env-dependencies",
      });
    ...
    it("does not change turbo.json if already migrated", async () => {
      // load the fixture for the test
      const { root, read, cleanup } = testTransform({
        fixture: "migrate-env-var-dependencies/migrated-env-dependencies",
      });
    ...
    it("errors if no turbo.json can be found", async () => {
      // load the fixture for the test
      const { root, read, cleanup } = testTransform({
        fixture: "migrate-env-var-dependencies/no-turbo-json",
      });
